...
def reset_evaluation_state():
    ...
    st.session_state.form_data = {
        ...
        "gender": "Чоловіча",
        "answers": {
            factor: [None] * len(questions) for factor, questions in QUESTIONS.items()
        },
        "question_comments": {
...
def build_ai_submission(form_data: dict):
    ...
    """
    from schemas import \
        TeacherFormSubmission  # pylint: disable=import-outside-toplevel
    ...
    for factor_name, (score_key, comments_key) in factor_mapping.items():
        ...
        submission_data[comments_key] = form_data["question_comments"].get(
            factor_name, []
        )
...
def calculate_factor_scores(form_data: dict, factor_names: list) -> dict:
    ...
    for factor in factor_names:
        ...
        numeric_answers = [int(a[0]) for a in answers if a and not a.startswith("NA")]

        if numeric_answers:
            calculated_scores[factor] = round(
                sum(numeric_answers) / len(numeric_answers)
            )
        else:
